...
db.exec(`
  CREATE TABLE IF NOT EXISTS users (
    ...
    feeStatus TEXT CHECK(feeStatus IN ('paid', 'unpaid')) DEFAULT 'unpaid',
    registeredSubjects TEXT -- JSON string
  );
...
try {
  db.prepare("ALTER TABLE users ADD COLUMN feeStatus TEXT CHECK(feeStatus IN ('paid', 'unpaid')) DEFAULT 'unpaid'").run();
} catch (e) {}
try {
  db.prepare("ALTER TABLE users ADD COLUMN registeredSubjects TEXT").run();
} catch (e) {}
...
async function startServer() {
  ...
  app.get('/api/users/me', authenticate, (req: any, res) => {
    const user = db.prepare('SELECT id, studentId, name, course, year, email, role, feeStatus, registeredSubjects FROM users WHERE id = ?').get(req.user.id);
    res.json(user);
  });

  app.post('/api/payments/pay-fees', authenticate, (req: any, res) => {
    try {
      db.prepare('UPDATE users SET feeStatus = ? WHERE id = ?').run('paid', req.user.id);
  ...
  app.post('/api/auth/register', (req, res) => {
    ...
    try {
      db.prepare('INSERT INTO users (studentId, name, course, year, email, password, role, feeStatus) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(
        studentId, name, course, year, email, hashedPassword, 'student', 'unpaid'
      );
  ...
  app.get('/api/users/students', authenticate, isAdmin, (req, res) => {
    const students = db.prepare('SELECT id, studentId, name, course, year, email, feeStatus, registeredSubjects FROM users WHERE role = ?').all('student');
    res.json(students);
  });

  app.post('/api/users/students', authenticate, isAdmin, (req, res) => {
    const { studentId, name, course, year, email, password, feeStatus, registeredSubjects } = req.body;
    const hashedPassword = bcrypt.hashSync(password, 10);
    try {
      db.prepare('INSERT INTO users (studentId, name, course, year, email, password, role, feeStatus, registeredSubjects) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
        studentId, name, course, year, email, hashedPassword, 'student', feeStatus || 'unpaid', JSON.stringify(registeredSubjects || [])
      );
  ...
  app.put('/api/users/students/:id', authenticate, isAdmin, (req, res) => {
    const { studentId, name, course, year, email, feeStatus, registeredSubjects } = req.body;
    try {
      db.prepare('UPDATE users SET studentId = ?, name = ?, course = ?, year = ?, email = ?, feeStatus = ?, registeredSubjects = ? WHERE id = ?').run(
        studentId, name, course, year, email, feeStatus, JSON.stringify(registeredSubjects || []), req.params.id
      );
  ...
  app.post('/api/exam-forms', authenticate, (req: any, res) => {
    ...
    // Check fee status
    const user = db.prepare('SELECT feeStatus FROM users WHERE id = ?').get(req.user.id) as any;
    if (user.feeStatus !== 'paid') {
      return res.status(403).json({ error: 'Please pay your fees before submitting the exam form' });
    }
    ...
    db.prepare('INSERT INTO exam_forms (userId, subjects) VALUES (?, ?)').run(
      req.user.id, JSON.stringify(subjects)
    );
  ...
  app.post('/api/allocate-seats', authenticate, isAdmin, (req, res) => {
    try {
      // Get all students with approved exam forms
      ...
      if (halls.length === 0) {
        return res.status(400).json({ error: 'No halls available for allocation' });
      }

      if (students.length === 0) {
        return res.status(400).json({ error: 'No approved students found for allocation' });
      }

      let studentIdx = 0;
      const insertAlloc = db.prepare('INSERT INTO seating_allocations (hallId, userId, seatNumber) VALUES (?, ?, ?)');
      const updateForm = db.prepare('UPDATE exam_forms SET seatNumber = ?, examCenter = ? WHERE userId = ?');

      const allocationTransaction = db.transaction(() => {
        // Clear existing allocations first
        db.prepare('DELETE FROM seating_allocations').run();
        
        for (const hall of halls) {
          for (let i = 1; i <= hall.capacity; i++) {
            if (studentIdx >= students.length) break;
            const student = students[studentIdx++];
            const seatNum = `${hall.roomNumber}-${i}`;
            const center = `Room ${hall.roomNumber}`;
            
            insertAlloc.run(hall.id, student.id, seatNum);
            updateForm.run(seatNum, center, student.id);
          }
          if (studentIdx >= students.length) break;
        }
      });
      ...
      res.json({ message: `Successfully allocated ${studentIdx} students` });
    } catch (error: any) {
      console.error('Allocation error:', error);
      res.status(500).json({ error: 'Failed to allocate seats: ' + error.message });
    }
  });
